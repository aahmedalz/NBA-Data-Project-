...
We were able to generate two complementary databases based upon publicly available data from the NBA’s public statistics repository. The first data source consisted of traditional team statistics, including standard per-game statistics ( points, rebounds, assists, turnovers/possessions), while the second data source consisted of advanced efficiency metrics (offensive rating, defensive rating, net rating, true shooting percentage, pace). Both databases contain 802 team/season records across 30 NBA teams for the period of 1996/97 through 2022/23. These two databases were joined together using a multi-key join based upon TEAM_ID and SEASON; the resulting data was then profiled and cleaned. Then two separate Random Forest Regression Models have been built and trained, with one using only traditional features and the other using only advanced features, to predict end-of-season winning percentage for each team.

Our findings are conclusive. The advanced stats model achieved an R² of 0.9336 and RMSE of 0.0399, explaining 93.36% of the variance in team win percentage. By contrast, the traditional stats model achieved R² of only 0.481 and RMSE of 0.1115, which is less than half the explanatory power. Among advanced metrics, NET_RATING, OFF_RATING, and DEF_RATING showed the highest individual correlations with W_PCT. Statistically, field goal percentage was the most predictive metric but was not as predictive as an efficiency rating. The comparison supports the analytics movement in professional basketball because it demonstrates quantitatively that efficiency ratings are dramatic predictors of team success over raw volume statistics.
